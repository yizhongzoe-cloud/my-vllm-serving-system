...
class Worker(WorkerBase):
    ...
    def query_restore_done(
        ...
        """V3 capacity-preempt: delegate async restore completion
        query to the model runner. Returns True if the async restore
    ...
    def register_rerouted_request(
        ...
        `output_token_ids` carries the per-token output sequence
        already generated by the origin engine (clamped to KV-covered
        range). Optional for back-compat with older callers; default
        [] = legacy prompt-end-resume behavior.
        """
        ...
        return fn(
            req_id, prompt_token_ids, sampling_params, num_computed_tokens,
            output_token_ids,
        )
