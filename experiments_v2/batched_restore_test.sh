#!/usr/bin/env bash
# Variant A: batched restore — cap restores-per-step to decouple GPU
# temp pressure from N_displaced. Uses gpu_util=0.9 (full KV pool),
# no per-req sync (full parallel within each batch), BATCH_SIZE=3.
#
# Test s456 first (19 displaced, worst case). If 100% comp + fg_p95
# reasonable, extend to 3-seed.
...
OUT="results_v2/8B/batched_restore_v1/456"
mkdir -p "$OUT"
...
echo "[$(date +%H:%M:%S)] Batched restore test (BATCH_SIZE=3), s456 (19 displaced)"
...
print(f's456 BATCH_SIZE=3: gp={m[\"goodput\"]:.1f} comp={m[\"completion_rate\"]*100:.0f}%% fg_p50={m.get(\"failover_gap_p50_ms\",0):.0f}ms fg_p95={m.get(\"failover_gap_p95_ms\",0):.0f}ms')
