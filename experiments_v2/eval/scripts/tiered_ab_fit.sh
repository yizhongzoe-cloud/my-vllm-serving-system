...
# apply). out-tag tiered3 so the buggy-config 'tiered' ours runs are
...
LOG_DIR="${EVAL_RESULTS_DIR}/logs/tiered_ab_fit3_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$LOG_DIR"
...
echo "[tiered_ab_fit] start $(date), logs in $LOG_DIR"
for cfg in "ours 0.5 9" "ours 0.5 0" "ours 0.4 9" "ours 0.4 0"; do
  set -- $cfg; bl=$1; q=$2; s=$3
  OUT="${EVAL_RESULTS_DIR}/dual_${bl}_arxivsumm_qps${q}_n60_seed${s}_tiered3_metrics.json"
  ...
  python -u experiments_v2/eval/scripts/dual_engine_microbench.py \
    ...
    --out-tag tiered3 > "${LOG_DIR}/${bl}_q${q}_s${s}.log" 2>&1
  echo "  exit=$? -> $(basename "$OUT")"
done
echo "[tiered_ab_fit] done $(date)"
