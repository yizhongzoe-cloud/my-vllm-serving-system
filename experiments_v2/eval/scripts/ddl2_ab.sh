...
LOG_DIR="${EVAL_RESULTS_DIR}/logs/ddl2_ab_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$LOG_DIR"
...
for cfg in "ours 0.4 16" "ours 0.5 16" "ours 0.5 9" "ours 0.5 0"; do
  ...
  OUT="${EVAL_RESULTS_DIR}/dual_${bl}_arxivsumm_qps${q}_n60_seed${s}_ddl2_metrics.json"
  ...
  python -u experiments_v2/eval/scripts/dual_engine_microbench.py \
    ...
    --out-tag ddl2 > "${LOG_DIR}/${bl}_q${q}_s${s}.log" 2>&1
