"""Sequential single-request microbenchmark for continuous-checkpoint
mechanism overhead.

Only one request is in flight at a time, so vLLM never triggers
capacity preemption. The only runtime difference between vllm_fcfs and
ckpt_only baselines is the host-RAM checkpoint write path.

Each trial uses a fresh pseudo-random prompt (defeats prefix caching);
prompts are long enough that the prefill produces many KV blocks, and
the requested output is long enough that the decode phase triggers
many continuous-checkpoint writes (one per 16-token block).

Output: experiments_v2/eval/results/a6000/ckpt_overhead_micro.json
"""
...
def make_prompt_tokens(seed: int, length: int) -> list[int]:
    """Pseudo-random tokens to defeat prefix caching across trials."""
    rng = random.Random(seed)
    return [rng.randint(1000, 100000) for _ in range(length)]


def send_request(prompt_tokens, max_tokens, trial_idx):
    """Send one streaming request, return TTFT/TPOT/E2E (ms)."""
    payload = {
        "model": MODEL,
...
def run_baseline(baseline, prompt_length, max_tokens, num_trials,
    ...
    print(f"[bench] {baseline} engine ready", flush=True)

    results = []
    try:
        for trial in range(num_trials):
            prompt = make_prompt_tokens(
                seed=trial * 9931 + (1 if baseline == "ckpt_only" else 0),
                length=prompt_length)
            print(f"[bench] {baseline} trial={trial}, sending "
                  f"{prompt_length}-tok prompt, {max_tokens} max_tokens...",
                  flush=True)
            r = send_request(prompt, max_tokens, trial)
            results.append(r)
            ttft = r["ttft_ms"]
            tpot = r["tpot_ms"]
            e2e = r["e2e_ms"]
            print(f"  ttft={ttft:.0f}ms tpot={tpot:.2f}ms "
                  f"e2e={e2e:.0f}ms tokens={r['num_tokens']}",
                  flush=True)
    finally:
        shutdown(proc)
        cleanup_state()
...
def main():
    ...
    log_dir = EVAL_RESULTS_DIR / f"logs/ckpt_overhead_{int(time.time())}"
    log_dir.mkdir(parents=True, exist_ok=True)

    print(f"[bench] prompt={args.prompt_length} max_tokens={args.max_tokens} "
          f"trials={args.num_trials} warmup={args.warmup}")
    ...
    for baseline in ("vllm_fcfs", "ckpt_only"):
        all_results[baseline] = run_baseline(
            ...
            args.num_trials, max_model_len, log_dir,
        )

    # Summary (skip warmup trials)
    import numpy as np
    print(f"\n[summary] median over trials {args.warmup}..{args.num_trials - 1}:")
    summary = {}
    for b in ("vllm_fcfs", "ckpt_only"):
        keep = all_results[b][args.warmup:]
