...
LOG_DIR="${EVAL_RESULTS_DIR}/logs/icept_cross_s1_$(date +%Y%m%d_%H%M%S)"; mkdir -p "$LOG_DIR"
...
Q=0.3; N=16; PMIN=6000; PMAX=12000; TOUT=60
echo "[cross] start $(date)"
run(){ # $1 baseline  $2 extra  $3 out-tag  $4 pause
  OUT="${EVAL_RESULTS_DIR}/icept_$1_qps${Q}_n${N}_seed1_$3_metrics.json"
  ...
  python -u experiments_v2/eval/scripts/icept_microbench.py \
    ...
    --pause-min "$4" --pause-max "$4" --seed 1 \
    --prompt-min-tokens "$PMIN" --prompt-max-tokens "$PMAX" \
    --out-tag "$3" > "${LOG_DIR}/$1_$3.log" 2>&1; echo "  exit=$?"; }
for P in 2 30 60; do
