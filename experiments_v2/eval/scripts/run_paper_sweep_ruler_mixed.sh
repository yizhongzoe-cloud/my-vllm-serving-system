#!/bin/bash
# RULER_MIXED paper sweep — E_M1 only (picker validation on long-context).
#
# Usage:
...
echo ""
echo "================================================"
echo "[master] E_M1 — RULER_MIXED rate sweep"
echo "================================================"
EXTRA_ARGS=""
if [ -n "${FORCE_OUTPUT_TOKENS}" ]; then
...
for seed in ${SEEDS}; do
  for qps in ${E_M1_QPS_SWEEP}; do
    for baseline in ${BASELINES_M1}; do
      echo "[master] E_M1 ruler_mixed baseline=${baseline} qps=${qps} seed=${seed}"
      cleanup_shm
