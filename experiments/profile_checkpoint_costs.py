...
class KVCheckpointBenchmark:
    ...
    - num_layers: 16 (hardcoded, can be extracted from model config if generalization needed)

    The benchmark measures actual KVCheckpointPool.save_checkpoint() and restore_checkpoint()
    operations to get realistic checkpoint and restore latencies for the cost model.
    """

    def __init__(self, num_kv_heads: int = 8, head_size: int = 64, block_size: int = 16):
        """Initialize with model geometry parameters.

        Args:
            num_kv_heads: Number of key-value attention heads (8 for Llama-3.2-1B)
            head_size: Size of each attention head in dimensions (64 for Llama-3.2-1B)
            block_size: KV cache block size in tokens (16, standard for most models)
        """
    ...
    def benchmark_save_restore(
        ...
        for num_blocks in block_counts:
            ...
            # Simulate 32 layers (like Llama-3.2-1B)
            num_layers = 32
...
def main():
    ...
    parser.add_argument("--output", default="experiments/checkpoint_cost_profile.json")
