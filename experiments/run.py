...
async def _send_single_request(
    ...
    if spec.failure_gap_slo_ms > 0 and result.max_gap_ms > spec.failure_gap_slo_ms:
        result.gap_violated = True
...
def main():
    parser = argparse.ArgumentParser(description="Single FT experiment run")
    ...
    # Resolve parameters.
    load_rps = config["load_levels"][args.load]
    fault_time_sec = config["fault_timing"].get(args.fault)
    workload_config = config["workloads"][args.workload]
    slo_config = config.get("slo", {})
    run_duration = config.get("run_duration_sec", 70.0)
    warmup_sec = config.get("warmup_sec", 5.0)
    timeout_sec = config.get("request_timeout_sec", 60.0)
    ...
    metadata = ExperimentMetadata(
        ...
        failure_gap_slo_ms=slo_config.get("failure_gap_ms", 0.0),
    )
